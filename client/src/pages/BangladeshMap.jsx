...
const FlyToDistrict = ({activeCoords}) => {
    const map = useMap()
    if (activeCoords){
        map.flyTo(activeCoords, 12, {duration: 2})
    }
    return null
}
const BangladeshMap = ({serviceCenters}) => {
    ...
    const handleSearch = e => {
        e.preventDefault()
        const district = serviceCenters.find(dst =>
        dst.district.toLowerCase().includes(searchText.toLowerCase())
        )
        if (district) {
    ...
    return (
        <div className="h-[800px] w-full rounded-lg overflow-hidden shadow-lg relative">
            ...
            <MapContainer center={position} zoom={13} scrollWheelZoom={true} className='h-full w-full z-0'>
                ...
                />
                <FlyToDistrict activeCoords={activeCoords}/>
                {
